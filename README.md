...
- `integration-reset.yml` — riallinea `integration` a `main` e ri-applica le feature ancora in-flight (PR aperta, non mergiata), matchando per chiave ticket. Input: `integration_branch`, `base_branch`, `ticket_prefix`, `deploy_workflow`, `dry_run`, `runner` (default `self-hosted`). Il caller DEVE concedere `pull-requests: read`. Cross-repo.
